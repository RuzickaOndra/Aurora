...
volatile uint32_t dataBuffer = 0;

volatile uint8_t dataReady = 0;

#define NUM_BINS 10 // Number of bins for ranges
#define MAX_PULSE_LENGTH 3000 // Maximum pulse length to consider

uint32_t pulseBins[NUM_BINS]; // Array to hold counts for each bin
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
	if (htim->Instance == TIM3 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
		...
		if (pulseWidth > 9000) { // Large gap (e.g., >10ms) indicates a new data frame
			...
			// If we have received 32 bits (NEC standard), decode and print
			if (bitIndex >= 32) {
				// decodeIR();
				dataReady = 1;
				bitIndex = 0;
